...
namespace MyLoad_Wap.LoadService
{
    public class MyRegister : MyLoadBase
    {
        ...
        public MyRegister(string MSISDN, string Para)
        {
            this.MSISDN = MSISDN;
            this.Para = Para;

            mTemplatePath = "~/Templates/Service/Register.htm";            
            Init();
        ...
        protected override string BuildHTML()
        {
            ...
            try
            {
                ...
                CommandCode = mTable.Rows[0]["RegKeyword"].ToString();
                ...
                {
                    Message = "Thông tin đăng ký dịch vụ (" + mTable.Rows[0]["ServiceName"].ToString() + ") cho số điện thoại (" + MSISDN + ") đã được gửi thành công đến hệ thống chờ xử lý.";
                }
                else
                {
                    Message = "Xin lỗi, hành động đăng ký dịch vụ không thành công, xin vui lòng thử lại sau ít phút.";
