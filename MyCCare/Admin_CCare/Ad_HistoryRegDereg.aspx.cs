...
{
    public partial class Ad_HistoryRegDereg : MyASPXBase
    {
        ...
        DataTable Admin_Paging1_GetData_Callback_Change()
        {
            try
            {
                ...
                DataTable mTable = mChargeLog.Search_SelectType(SearchType, Admin_Paging1.mPaging.BeginRow, Admin_Paging1.mPaging.EndRow, SearchContent, PID,ServiceID, 0, 0, 0, BeginDate, EndDate, 1, SortBy);
                DataColumn mCol_1 = new DataColumn("ActionName", typeof(string));
                mTable.Columns.Add(mCol_1);

                foreach (DataRow mRow in mTable.Rows)
                {
                    if ((int)mRow["ChargeTypeID"] == (int)ChargeLog.ChargeType.REG)
                    {
