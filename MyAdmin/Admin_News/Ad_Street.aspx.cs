...
{
    public partial class Ad_Street : System.Web.UI.Page
    {
            ...
            try
            {
                switch (type)
                {
                    case 1://Bind Position
                        ...
                        sel_Position.Items.Insert(0, new ListItem("--Địa điểm--", "0"));
                        break;
                }
            }
            catch (Exception ex)
        ...
        int Admin_Paging1_GetTotalPage_Callback_Change()
        {
            try
            {
                ...
                if (sel_Position.SelectedIndex > 0)
                {
                    int.TryParse(sel_Position.Value, out PositionID);
                ...
                return mStreet.TotalRow(SearchType, str_SearchContent,PositionID, IsActive);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        DataTable Admin_Paging1_GetData_Callback_Change()
        {
            try
            {
                ...
                return mStreet.Search(SearchType, Admin_Paging1.mPaging.BeginRow, Admin_Paging1.mPaging.EndRow, str_SearchContent,PositionID, IsActive, SortBy);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        protected void lbtn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mStreet.Delete(0, dds_Parent.GetXml()))
                {
                    #region Log member
                    MemberLog mLog = new MemberLog();
                    MemberLog.ActionType Action = MemberLog.ActionType.Delete;
                    mLog.Insert("Street", string.Empty, dds_Parent.GetXml(), Action, true, string.Empty);
                    #endregion
        ...
        private void Active(bool IsActive)
        {
            try
            {
                ...
                if (mStreet.Active(0, IsActive, dds_Parent.GetXml()))
                {
                    #region Log member
                    MemberLog mLog = new MemberLog();
